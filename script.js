...
document.querySelectorAll('.nav a').forEach(link=>link.addEventListener('click',()=>{menu.classList.remove('open');menuToggle.setAttribute('aria-expanded','false');}));
...
form.addEventListener('submit',event=>{
  ...
  dialog.close();form.reset();toast.classList.add('show');setTimeout(()=>toast.classList.remove('show'),2600);
});
...
function filterDictionary(){
  const q=search.value.trim().toLowerCase();const c=category.value;let visible=0;
  cards.forEach(card=>{const show=card.dataset.word.includes(q)&&(c==='all'||card.dataset.category===c);card.hidden=!show;if(show)visible++;});
